"""RL v5-alt: same as v5 but different seed ranges (parallel extra search)."""
...
OUT_FILE = RESULTS_DIR / 'rl_v5_alt.json'

RATE_WEIGHTS = {1.0: 1.0, 2.0: 1.0, 3.0: 1.0, 4.0: 2.0}
SEEDS_GREEDY = list(range(200, 208))  # different range from v5
SEEDS_FBFLY  = list(range(300, 308))
TOP_K = 3
N_EPISODES = 1000
ENTROPY_COEF = 0.01
...
def main():
    ...
    cfg_adj = f'v5alt_{wl}_{label}_adj'; mw.gen_anynet_config(cfg_adj, grid, adj_c, chip_n=N, outdir=mw.CONFIG_DIR)
    ...
    cfg_g = f'v5alt_{wl}_{label}_greedy'; mw.gen_anynet_config(cfg_g, grid, g_c, chip_n=N, outdir=mw.CONFIG_DIR)
    fb_alloc = flattened_butterfly_alloc(grid, budget, per_pair_cap=N, max_dist=max_dist)
    fb_c = {p: min(n, N) for p, n in fb_alloc.items()}
    cfg_fb = f'v5alt_{wl}_{label}_fbfly'; mw.gen_anynet_config(cfg_fb, grid, fb_c, chip_n=N, outdir=mw.CONFIG_DIR)

    surrogate = load_surrogate_v2()
    candidates = []
    for warm_name, warm_alloc, seeds in [('greedy', g_c, SEEDS_GREEDY), ('fbfly', fb_c, SEEDS_FBFLY)]:
        for seed in seeds:
            torch.manual_seed(seed); np.random.seed(seed)
            top_k = mw.train_warmstart_rl_ra(
                surrogate, wl, K, N, R, C, bpp,
                n_episodes=N_EPISODES, rate_mult=4.0,
                rate_weights=RATE_WEIGHTS, warm_start_alloc=warm_alloc,
                entropy_coef=ENTROPY_COEF, top_k=TOP_K)
            for k_idx, (alloc, pred, _) in enumerate(top_k):
                rl_c = {p: min(n, N) for p, n in alloc.items()}
                cfg = f'v5alt_{wl}_{label}_{warm_name}_s{seed}_k{k_idx}'
    ...
    result['ours_v5alt'] = {'latency': best['latency'], 'throughput': best['throughput'], 'best_candidate': best_tag}
    print(f'    ours_v5alt: best={best_tag}, max={max(best["latency"]):.1f}, dt={time.time()-t0:.0f}s', flush=True)
