...
We first prove that center-link amplification under adjacent-only routing grows as $\Theta(K^{3/2})$ and that no routing algorithm eliminates this structural tax, motivating express links as a topological rather than allocation-level fix. Across \textbf{28 BookSim configurations} spanning seven LLM-relevant workloads (NL\% from 10\% to 91\%) and two chiplet counts $K\!\in\!\{16,32\}$ at per-chiplet mesh sizes $N\!\in\!\{4,8\}$, our \textbf{RL-WS} method---multi-warm-start REINFORCE with rate-aware surrogate v2 and top-$k$ BookSim validation---improves latency by \textbf{35.6\%} on average over adjacent-uniform no-express baselines, exceeding both the workload-aware greedy heuristic (+25.6\%) and a Flattened-Butterfly (FBfly) topology-aware allocator that is the strongest hand-engineered baseline. RL-WS strictly beats FBfly on \textbf{24/28} cells, ties on 2, and stays within simulation noise on the remaining 2; the best single cell reaches \textbf{$-83.2\%$} latency vs FBfly on MoE Skewed, $K$=32, $N$=8, $b$=4$\times$ (67.8 vs 402.8 cycles). Pooled across the 28 cells, NL\% predicts RL-WS saving with Spearman $\rho{=}0.83$ ($p=6.8\times 10^{-8}$). The pattern is sharply NL\%-stratified: on the 16 high-NL cells (NL$\ge$77\%, including MoE/Hybrid TP+PP/Uniform Random/All-to-all), RL-WS strictly dominates FBfly; on the 12 low-NL cells (Ring/Pipeline/Tree at NL$\le$50\%), FBfly's regular row/column allocation is already near-optimal and RL-WS matches it within $\pm$1\,cycle. NL\% therefore acts as a \textit{deployment classifier}---a single static statistic that tells architects when traffic-aware learned placement is essential versus when a topology-aware heuristic suffices.
\end{abstract}
...
\textbf{Final BookSim selection.} All 48 candidates plus the three baselines (adj-uniform, greedy, FBfly) are simulated in BookSim at the four training rates, and the candidate with the lowest max-rate latency is selected. This makes the final selection \textit{measured} rather than surrogate-predicted: surrogate error cannot produce a silent regression because every shortlisted candidate is end-to-end simulated.
...
\textbf{(2) FBfly is near-optimal on low-NL workloads.} On the three low-NL\% groups (NL$\le$50\%: Tree, Ring, Pipeline), demand is concentrated on adjacent pairs and FBfly's row/column regularity is essentially optimal; RL-WS matches FBfly within $\pm$1\,cycle on 10/12 cells (8 strict beats, 2 ties, 2 minor losses by $\le$0.9\,cycle).
...
The aggregate numbers in Table~\ref{tab:main_result} hide three large per-cell wins that drive the overall mean. We highlight them because they characterize \textit{when} RL-WS extracts dramatic value relative to FBfly:
\begin{itemize}
\item \textbf{MoE Skewed, $K{=}32$, $N{=}8$, $b{=}4\times$}: 67.8 vs 402.8 cycles ($-83.2\%$). FBfly's symmetric row/column allocation under-provisions the few high-traffic expert pairs in skewed top-2 dispatch, while RL-WS concentrates capacity on those pairs.
\item \textbf{MoE Skewed, $K{=}32$, $N{=}4$, $b{=}4\times$}: 42.2 vs 272.9 cycles ($-84.5\%$). Same mechanism at the smaller per-pair cap.
\item \textbf{MoE Skewed, $K{=}16$, $N{=}8$, $b{=}4\times$}: 54.3 vs 198.6 cycles ($-72.6\%$).
\end{itemize}
These three cells alone contribute most of the $-11.8\%$ overall latency reduction vs FBfly. Outside MoE, the typical reduction is $3$--$5\%$. This is the empirical answer to ``when is workload-aware RL placement worth the cost'': when traffic is both \textit{highly non-local} and \textit{strongly skewed}, regular topology-aware allocation under-provisions the heavy-traffic pairs and an adaptive placer pays for itself by orders of magnitude.

\textbf{Where RL-WS does not strictly beat FBfly.} On 4/28 cells (Tree $K{=}16, N{=}4$, Tree $K{=}32, N{=}8$ at $b{=}2\times$, Pipeline $K{=}16, N{=}4$, Ring $K{=}16, N{=}4$) RL-WS does not strictly beat FBfly: 2 are ties within $\pm$0.1\,cycle and 2 are minor losses by $\le$0.9\,cycle. All four are NL$\le$50\% cells where adjacent demand dominates and FBfly's regular allocation already maps onto the actual phantom-load pattern; in this regime RL-WS produces equivalent allocations within the stochasticity of REINFORCE+surrogate. This is the empirical justification for using NL\% as a deployment classifier: at NL$\le$50\% there is no consistent measurable gap to chase.
...
\textbf{Limitations.} (i) Our workloads are synthetic but workload-inspired rather than traced from full training runs. (ii) RL-WS uses 16 random seeds (8 greedy-warm + 8 FBfly-warm) per cell internally, but final BookSim selection picks one realized allocation per cell; we do not report per-cell BookSim variance over independent end-to-end RL re-runs and treat such multi-run variance as future work. (iii) PARL~\cite{parl} targets a complementary problem (multi-tenant interference minimization via full topology synthesis on heterogeneous MI300X-class chiplets with the Interference Score metric); we therefore do not attempt a head-to-head reproduction of PARL on our single-tenant, mean-latency, homogeneous-grid setup, and position it qualitatively instead. (iv) The express-link wire-delay model is linear and analytical; we validate robustness to this assumption via the $\lambda\!\in\!\{1.0,1.5,2.0\}$ sweep in Table~\ref{tab:lambda_sensitivity} but do not derive per-link latencies from SPICE-level physical simulation. (v) The NL\% predictor is evaluated against XY-style shortest-path routing on the chiplet-level graph and may behave differently under adaptive routing. (vi) The paper targets 2.5D chiplet integration (side-by-side dies on a silicon interposer, as in CoWoS-S/L or EMIB); extending the express-link formulation to 3D die-stacked packages (Foveros-style) with TSV-dominant wire models is future work. None of these undermines the core claim that NL\% is a deployment classifier that distinguishes when traffic-aware learned placement (RL-WS) is essential from when topology-aware regular allocation (FBfly) suffices.

% ============================================================
\section{Conclusion}
% ============================================================

\textbf{Conclusion.} We cast express-link placement for LLM chiplet networks as a three-step problem---\textsc{Predict}, \textsc{Place}, \textsc{Refine}---and show that a single static workload statistic, the non-locality fraction NL\%, acts as a \textit{deployment classifier} for the third step. NL\% predicts express-link saving with Spearman $\rho=0.83$ across 28 workload-configuration cells covering seven LLM-relevant traffic patterns, and---more importantly---it sharply stratifies when learned placement is worth invoking: on 16/16 high-NL cells (NL$\ge$77\%) RL-WS strictly beats the strongest topology-aware baseline (FBfly) with a mean reduction of $-19.0\%$ in latency and a peak reduction of $-83.2\%$ on MoE Skewed $K{=}32, N{=}8, b{=}4\times$; on 12 low-NL cells (NL$\le$50\%) FBfly's regular row/column allocation already captures most of the benefit and RL-WS matches it within simulation noise. Across all 28 cells RL-WS attains a 35.6\% mean latency saving versus adjacent-uniform, exceeding both greedy (25.6\%) and FBfly (27.1\%) and strictly winning 24/28 head-to-head against FBfly. Together, these results justify a practical chiplet NoI workflow: compute NL\% first, place with FBfly as a strong deterministic default, and invoke RL-WS whenever NL\%$\ge$77\% indicates that the workload's non-local skew leaves meaningful headroom for traffic-aware adaptation.
