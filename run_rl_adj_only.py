"""Adj-only RL: max_dist=1 — RL에 hop=1 (adjacent) link만 허용.

Hypothesis: same workload-aware allocation but no long-distance express →
much smaller wire-mm² but how does latency compare to FBfly?

Usage: .venv/bin/python3 run_rl_adj_only.py <workload> <K> <N> <bpp>
"""
import json
import sys
import time
from pathlib import Path
import numpy as np
import torch
import torch.nn as nn

import ml_express_warmstart as mw

RESULTS_DIR = Path('results/ml_placement')
OUT_FILE = RESULTS_DIR / 'rl_adj_only.json'

RATE_WEIGHTS = {1.0: 1.0, 2.0: 1.0, 3.0: 1.0, 4.0: 2.0}
SEEDS = list(range(42, 42 + 8))  # 8 seeds (adj-only has smaller search space)
TOP_K = 3
N_EPISODES = 1000
ENTROPY_COEF = 0.01


def load_surrogate_v2():
    class RAv2(nn.Module):
        def __init__(self, input_dim=501, hidden=256):
            super().__init__()
            self.net = nn.Sequential(
                nn.Linear(input_dim, hidden), nn.ReLU(), nn.LayerNorm(hidden),
                nn.Linear(hidden, hidden), nn.ReLU(), nn.LayerNorm(hidden),
                nn.Linear(hidden, 64), nn.ReLU(), nn.Linear(64, 1))
        def forward(self, x): return self.net(x).squeeze(-1)
    m = RAv2(501).to(mw.DEVICE)
    m.load_state_dict(torch.load(RESULTS_DIR / 'surrogate_rate_aware_v2.pt', map_location=mw.DEVICE))
    m.eval()
    return m


def main():
    wl = sys.argv[1]; K = int(sys.argv[2]); N = int(sys.argv[3]); bpp = int(sys.argv[4])
    print(f'>>> ADJ-ONLY {wl} K{K}N{N} b{bpp}× (max_dist=1, {len(SEEDS)} seeds, top-{TOP_K})', flush=True)
    t0 = time.time()

    R, C = (4, 4) if K == 16 else (4, 8)
    grid = mw.ChipletGrid(R, C)
    print(f'  [t={time.time()-t0:.1f}s] grid built', flush=True)
    traffic = mw.WORKLOADS[wl](K, grid)
    print(f'  [t={time.time()-t0:.1f}s] traffic generated', flush=True)
    adj_pairs = grid.get_adj_pairs()
    n_adj = len(adj_pairs)
    budget = int(n_adj * bpp)
    npc = N * N
    base_rate = mw.TOTAL_LOAD_BASE / (K * npc)
    rates = [base_rate * (i + 1) for i in range(4)]

    label = f'K{K}_N{N}_bpp{bpp}'
    traf = f'traffic_adjonly_{wl}_{label}.txt'
    mw.gen_traffic_matrix(grid, traffic, npc, mw.CONFIG_DIR / traf)

    # Adj-only baselines
    per = budget // n_adj; res = budget - per * n_adj
    adj_alloc = {p: per + (1 if i < res else 0) for i, p in enumerate(sorted(adj_pairs))}
    adj_c = {p: min(n, N) for p, n in adj_alloc.items()}
    cfg_adj = f'adjonly_{wl}_{label}_adj'
    mw.gen_anynet_config(cfg_adj, grid, adj_c, chip_n=N, outdir=mw.CONFIG_DIR)

    print(f'  [t={time.time()-t0:.1f}s] starting adj-only greedy (max_dist=1)', flush=True)
    g_alloc = mw.alloc_express_greedy(grid, traffic, budget, max_dist=1)
    g_c = {p: min(n, N) for p, n in g_alloc.items()}
    cfg_g = f'adjonly_{wl}_{label}_greedy'
    mw.gen_anynet_config(cfg_g, grid, g_c, chip_n=N, outdir=mw.CONFIG_DIR)
    print(f'  [t={time.time()-t0:.1f}s] adj-only greedy done, configs written', flush=True)

    print(f'    Budget={budget}, n_adj={n_adj}, loading surrogate...', flush=True)
    surrogate = load_surrogate_v2()
    print(f'  [t={time.time()-t0:.1f}s] surrogate loaded', flush=True)

    # Adj-only RL: 8 seeds × top-3 = 24 candidates
    candidates = []
    for seed in SEEDS:
        torch.manual_seed(seed); np.random.seed(seed)
        top_k = mw.train_warmstart_rl_ra(
            surrogate, wl, K, N, R, C, bpp,
            n_episodes=N_EPISODES, rate_mult=4.0,
            rate_weights=RATE_WEIGHTS,
            warm_start_alloc=g_c,
            entropy_coef=ENTROPY_COEF,
            top_k=TOP_K,
            max_dist=1,  # ADJ-ONLY
        )
        for k_idx, (alloc, pred, _) in enumerate(top_k):
            rl_c = {p: min(n, N) for p, n in alloc.items()}
            cfg = f'adjonly_{wl}_{label}_s{seed}_k{k_idx}'
            mw.gen_anynet_config(cfg, grid, rl_c, chip_n=N, outdir=mw.CONFIG_DIR)
            candidates.append((f's{seed}_k{k_idx}', rl_c, cfg))
    print(f'    Generated {len(candidates)} adj-only RL candidates', flush=True)

    # BookSim measurement
    result = {
        'workload': wl, 'K': K, 'N': N, 'budget_per_pair': bpp,
        'rates': rates,
        'adj_uniform': {'latency': [], 'throughput': []},
        'adj_greedy': {'latency': [], 'throughput': []},
        'candidates': {},
    }
    for method, cfg in [('adj_uniform', cfg_adj), ('adj_greedy', cfg_g)]:
        for rate in rates:
            r = mw.run_booksim(cfg, traf, rate, timeout=900)
            result[method]['latency'].append(r['latency'])
            result[method]['throughput'].append(r['throughput'])
        print(f'    {method}: max_lat={max(x for x in result[method]["latency"] if x is not None):.1f}', flush=True)

    for tag, alloc, cfg in candidates:
        lats, tps = [], []
        for rate in rates:
            r = mw.run_booksim(cfg, traf, rate, timeout=900)
            lats.append(r['latency']); tps.append(r['throughput'])
        result['candidates'][tag] = {'latency': lats, 'throughput': tps}

    def _safe_max(lats):
        vals = [x for x in lats if x is not None]
        return max(vals) if vals else float('inf')

    best_tag = min(result['candidates'].keys(),
                    key=lambda t: _safe_max(result['candidates'][t]['latency']))
    best = result['candidates'][best_tag]
    result['ours_adj_only'] = {
        'latency': best['latency'], 'throughput': best['throughput'],
        'best_candidate': best_tag,
    }
    dt = time.time() - t0
    print(f'    ours_adj_only: best={best_tag}, max_lat={_safe_max(best["latency"]):.1f}, elapsed={dt:.0f}s', flush=True)

    existing = json.load(open(OUT_FILE)) if OUT_FILE.exists() else []
    existing = [r for r in existing if (r['workload'], r['K'], r['N'], r['budget_per_pair']) != (wl, K, N, bpp)]
    existing.append(result)
    with open(OUT_FILE, 'w') as f:
        json.dump(existing, f, indent=2)
    print(f'>>> DONE', flush=True)


if __name__ == '__main__':
    main()
