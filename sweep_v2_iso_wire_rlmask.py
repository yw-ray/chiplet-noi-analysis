...
def run_one(label, alloc, K, N, R, C, w_name, rate_mult=4.0):
    ...
    cfg = f"v2iso_rlmask_{label}_{w_name}_K{K}N{N}"
    traf_file = f"traffic_v2iso_rlmask_{label}_{w_name}_K{K}N{N}.txt"
...
def main():
    ...
    rl_data = json.loads((RESULTS_DIR / 'masks_rl.json').read_text())
    masks = {w: {tuple(int(x) for x in k.split('-')): v
                 for k, v in m.items()}
             for w, m in rl_data['masks_rl'].items()}
    ...
    out_path = RESULTS_DIR / 'sweep_v2_iso_wire_rlmask.json'
